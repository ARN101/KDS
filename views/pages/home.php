...
$total_members = (int) $db->query("SELECT COUNT(*) FROM members")->fetchColumn() + 30; 
$total_achievements = (int) $db->query("SELECT COUNT(*) FROM achievements")->fetchColumn();
$total_debates = (int) $db->query("SELECT COUNT(*) FROM debates")->fetchColumn() + 25; 
    ...
    <div class="max-w-7xl mx-auto px-6 md:px-12">
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-12 text-center" id="counters-container">
            ...
            <div class="flex flex-col items-center">
                <span class="font-serif text-4xl md:text-6xl font-bold tracking-tight text-white mb-2" data-target="<?= $total_members ?>"><?= $total_members ?></span>
                <span class="text-[10px] uppercase tracking-[0.2em] text-gray-500 font-semibold">Active Champions</span>
            </div>

            
            <div class="flex flex-col items-center">
                <span class="font-serif text-4xl md:text-6xl font-bold tracking-tight text-brandRed mb-2" data-target="<?= $total_achievements ?>"><?= $total_achievements ?></span>
                <span class="text-[10px] uppercase tracking-[0.2em] text-gray-500 font-semibold">National Trophies</span>
            </div>

            
            <div class="flex flex-col items-center">
                <span class="font-serif text-4xl md:text-6xl font-bold tracking-tight text-white mb-2" data-target="<?= $total_debates ?>"><?= $total_debates ?></span>
                <span class="text-[10px] uppercase tracking-[0.2em] text-gray-500 font-semibold">Chamber Debates</span>
            </div>

            
            <div class="flex flex-col items-center">
                <span class="font-serif text-4xl md:text-6xl font-bold tracking-tight text-brandGreen mb-2" data-target="<?= $total_events ?>"><?= $total_events ?></span>
                ...
                <?php foreach ($featured_debates as $debate): ?>
                    ...
                    <div class="spotlight-card glass-panel glass-panel-hover p-8 flex flex-col justify-between min-h-[380px] rounded group transition-all duration-300">
                        ...
                        <div>
                            ...
                            <a href="<?= e(get_youtube_url($debate['video_url'])) ?>" target="_blank" class="inline-flex items-center gap-2 text-xs uppercase tracking-widest font-bold text-white group-hover:text-brandRed transition-colors duration-300 mt-2">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
                ...
                <div class="lg:col-span-5 flex flex-col items-center justify-center text-center">
                    <div class="w-full max-w-sm glass-panel bg-brandBlack/50 p-8 border-white/5 rounded-lg flex flex-col items-center">
                        ...
                        <div class="flex items-center gap-4 text-white mb-8" id="countdown-timer" data-timestamp="<?= strtotime($upcoming_event['event_date'] . ' ' . $upcoming_event['event_time']) * 1000 ?>">
...
<script>
    ...
    if (timerContainer) {
        const targetDate = parseInt(timerContainer.getAttribute('data-timestamp'), 10);
